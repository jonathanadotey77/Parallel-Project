  ...
  for(int n = 1; n <= 16; n <<= 1) {
    ...
    for(int g = 0; g < num_gpus; ++g) {
      ...
      while(inFile >> num_nodes) {
        ...
        assert(num_nodes == n);
        //Send tuple to DB
        conn.insertKnapsackTime(num_nodes, node, round, balance, time);
      }
      inFile.close();
    }
    ...
    for(int w = 0; w < num_workers; ++w) {
      ...
      while(inFile >> num_nodes) {
        ...
        assert(num_nodes == n);
        //Send tuple to DB
        conn.insertRoundTime(num_nodes, round, time);
      }
      inFile.close();
    }
    ...
    for(int w = 0; w < num_workers; ++w) {
      ...
      while(inFile >> num_nodes) {
        ...
        assert(num_nodes == n);
        //Send tuple to DB
        conn.insertInvestorStatus(num_nodes, node, round, id, strategy, aggressiveness, market, balance);
      }
